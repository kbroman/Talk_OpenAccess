...
\onslide<4->{
  ...
  \ig[height=0.35\textheight]{Images/feingold_appendix.png}
}
...
\begin{frame}{Further \only<1|handout 0>{topics}\only<2>{\color{vhilight} rants}}

\vspace{36pt}

\bi
\itemsep12pt
\item More on PubMed Central
\item Books/monographs
\item Edited books
\item Over-emphasis on peer-reviewed articles
\item Supplements
\ei

\note{\scriptsize I could go on, but I should stop and give time for
  discussion.

  Here are a few of the things that I had to leave out.

  PMC is annoying for several reasons: It's painful to try to
  incorporate changes from the copy editing process back into one's
  manuscript. It's not clear what you're supposed to do with the
  copyright release forms. The one year embargo, and the incomplete
  coverage, severely limits the value of PMC.

  Books and monographs are way too expensive, publishers add little
  value, and authors sign away all control.

  Edited books are too expensive, largely unavailable and unread, and
  are are a complete waste of time, in their present form. Why not
  skip the publishers completely?

  There may be an over-emphasis on peer-reviewed journal articles in
  evaluating a scholar's productivity. This is particularly true for
  computational scientists. The value of less formal publications, and
  of things like software tools, should be recognized.

  Supplements can be a pain: extra effort to obtain these pieces,
  sometimes behind paywall, sometimes you have to download a dozen
  separate files, often hardly copy-edited, and sometimes only as html
  (not part of the PDF).
}
\end{frame}
