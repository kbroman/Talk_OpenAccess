...
\begin{frame}{All \only<1>{I}\only<2>{{\color{hilight} we}} care about}

\bi
\itemsep12pt
\item Getting the papers \only<1>{I}\only<2>{{\color{hilight} we}} want to read
\item People reading \only<1>{my}\only<2>{{\color{hilight} our}} papers
\ei
\end{frame}
